...
import com.jiao.util.HttpUtil;
import com.jiao.util.Pkipair;
...
public class KqService {
    ...
    //准备快钱支付接口的参数,map存储参数值
    public Map<String, String> generateKqForData(Integer userId, String rechargeNo, String rechargeMoney) {
        ...
        String bgUrl = "http://47.113.198.114:9999/pay/kq/notifty";
        //网关版本，固定值：v2.0,该参数必填。
        String version = "v2.0";
    ...
    public void handlerKQNotify(HttpServletRequest request) {
        ...
        if (flag) {
            ...
            result = Result.fail();
            result.setEnum(CodeEnum.RC_RECHARGE_SIGN_FAIL);
        }
                ...
                if (jsonObject != null) {
                    if (jsonObject.get("orderDetail") == null) {
                        ...
                        for (int i = 0; i < detailArray.size(); i++) {
                            ...
                            //获取payResult,payAmount,orderId,添加订单记录
                            result = rechargeService.handlerRechargeNotify(detail.getString("orderId"),
